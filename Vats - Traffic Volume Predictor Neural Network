...
y_normal = y_scaled.fit_transform(y.reshape(-1, 1))
...
def train_nn(neural_network, criterion, optimizer, testloader, trainloader, device, max_iters):
    ...
    # Training and testing loss for plotting
    ...
    for iter in range(max_iters):
        ...
        if (iter + 1) % 10 == 0 or iter == 0:
            print(f"Iteration: {iter+1} - Training Loss: {avg_train_loss:.4f}, Testing Loss: {avg_test_loss:.4f}, Learning Rate: {current_lr:.8f}")

        # Plot Learning Rate Over 5000 Epochs
        lr_values = []
    ...
    # Plot Cosine Annealing Scheduler (Cosine Wave)
    plt.figure(figsize=(12, 4))
    plt.plot(np.arange(5000), lr_values, label="Learning Rate")
    plt.title("Cosine Annealing LR Schedule (Learning Rate Period = 5000, Minimum Learning Rate = 1e-8)")
    plt.xlabel("Epochs")
...
train_nn(neural_network, criterion, optimizer, testloader, trainloader, device, max_iters=30000)
